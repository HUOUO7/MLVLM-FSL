...
def sampled_json(data_root, dataset_dir, split_dir, class_dict, dataset_name, n_way, k_shot, k_query, rotate_dir,
                 num_episodes=None):
    ...
    if num_episodes is None:
        num_episodes = max(12000, num_classes * 10)  # how many tasks to sample
    ...
    for i in tqdm.tqdm(range(num_episodes)):
        ...
        for way in support_way_shot_dict:
            ...
            for shot in support_way_shot_dict[way]:
                ...
                output_text += f"Picture {count}: <img>{img_path}</img>\n What is this? {label}.\n "
        ...
        for query_way in query_way_shot_dict:
            ...
            for query_shot in query_way_shot_dict[query_way]:
                # init
                conversation = {}
                conversation['id'] = "identity_" + str(len(all_samples)+1)
                ...
                query_idx = n_way+1
                query_text += f"Query Picture {query_idx}: <img>{query_img_path}</img>\n So what is this? Output is one of {label_list}. Make sure your output is in the answer list, with no spelling mistakes."
                # user
                user_dialog = {}
                user_dialog['from'] = 'user'
                user_dialog['value'] = query_text
                conversation['conversations'].append(user_dialog)
                # assistant
                assistant_dialogue = {}
                assistant_dialogue['from'] = 'assistant'
                assistant_dialogue['value'] = f"{query_label}."
                conversation['conversations'].append(assistant_dialogue)
...
all_datasets_name = ['oxford_iiit_pets_20211007', 'eurosat_clip_20210930', 'dtd_20211007', 'food_101_20211007',
                     'resisc45_clip_20210924', 'country211_20210924', 'oxford_flower_102_20211007',
                     'stanford_cars_20211007', 'mnist_20211008', 'fgvc_aircraft_2013b_variants102_20211007',
                     'fer_2013_20211008', 'gtsrb_20210923']
to_simple = {'oxford_iiit_pets_20211007': 'iiit_pets', 'eurosat_clip_20210930': 'eurosat_clip', 'dtd_20211007': 'dtd',
             'food_101_20211007': 'food101', 'resisc45_clip_20210924': 'resisc45', 'country211_20210924': 'country211',
             'oxford_flower_102_20211007': 'flower102', 'stanford_cars_20211007': 'stanford_cars',
             'caltech_101_20211007': 'caltech_101', 'mnist_20211008': 'mnist',
             'fgvc_aircraft_2013b_variants102_20211007': 'fgvc_aircraft', 'fer_2013_20211008': 'fer2013',
             'gtsrb_20210923': 'gtsrb'}
...
for dataset_name in all_datasets_name:
    ...
    if exchange_flag == True:
        flag = 'exchange'
        with open(
                f"/home/user4/cww/Elevater_Toolkit_IC-main/A_Qwen_format/aug_label/exchange_label/exchange_{dataname}_{split}.json",
                "r") as f:
        ...
        flag = 'pseudo'
        with open(
                f"/data/user4/cww/Data_Construction/fsl_data/pseudo_elevater_label_new/{dataname}_pseudo_{split}_label.json",
                "r") as f:
            class_dict = json.load(f)
    else:
        with open(f"/data/user4/cww/Data_Construction/fsl_data/elevater_label_new/{dataname}_{split}_label.json") as f:
            class_dict = json.load(f)
    out_dir = f'/data/user4/cww/Data_Construction/output7_newformat_Qwen_cww/novel_ele_{n_way}way/{flag}_ele_{num}/{split}_all/'
    os.makedirs(out_dir, exist_ok=True)
    output_json = f'{out_dir}{dataname}_{split}_{n_way}way{k_shot}shot{k_query}query.json'

    try:
        sampled_json(data_root, dataset_dir, split_dir, class_dict, dataset_name, n_way, k_shot, k_query, rotate_dir,
                     num)
