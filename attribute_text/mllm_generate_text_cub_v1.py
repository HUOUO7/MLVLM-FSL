        ...
        for class_name in tqdm(os.listdir(split_path), desc='Processing'):
            ...
            for file in tqdm(os.listdir(class_path), desc='Processing'):
                if file.endswith('.jpg'):
                    ...
                    color_query = f'Picture : <img>{image_path}</img>\n Please describe in detail the main colors and color distribution of each part of the bird\'s body (such as the head, back, abdomen, wings and tail, etc.) shown in this picture from the perspective of color pattern. Please give a rich paragraph answer and keep your answer to no more than 100 words.'
                    color_pattern_res = get_description(model, tokenizer, color_query)
                    print(f"Color pattern: {color_pattern_res}")

                    body_size_query = f'Picture : <img>{image_path}</img>\n Please describe the size and shape of the bird in this picture in detail, including the shape of its head, beak, wings, tail, legs, etc. Please give a rich paragraph answer and keep your answer to no more than 100 words.'
                    body_shape_and_size_res = get_description(model, tokenizer, body_size_query)
                    print(f"Body shape and size: {body_shape_and_size_res}")

                    feather_query = f'Picture : <img>{image_path}</img>\n Please describe the type and structure of the feathers of the bird shown in this picture in detail from the perspective of feather characteristics, including the length, texture, and arrangement of the feathers, etc. Please give a rich paragraph answer and keep your answer to no more than 100 words.'
                    feather_characteristics_res = get_description(model, tokenizer, feather_query)
                    print(f"Feather characteristics: {feather_characteristics_res}")

                    behavioral_query = f'Picture : <img>{image_path}</img>\n Please describe the behavior and posture of the bird shown in this picture in detail from the perspective of behavioral characteristics, such as standing, flying, foraging or other activities, etc. Please give a rich paragraph answer and keep your answer to no more than 100 words.'
                    behavioral_characteristics_res = get_description(model, tokenizer, behavioral_query)
                    print(f"Behavioral characterist ics: {behavioral_characteristics_res}")
                    ...
                    class_dict['description_data'].update({
                        image_name: {
                            ...
                            'behavioral_characteristics': behavioral_characteristics_res,
